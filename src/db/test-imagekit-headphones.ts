...
// Color-aware grouping for headphones
...
async function testHeadphonesGrouping() {
  console.log("🧪 Testing Headphones Grouping with Color Variants...\n");

  const result = await getImagesFromFolder("Headphones");

  if (result.success) {
    const grouped = groupHeadphonesByModel(result.files);

    console.log(
      `🎧 Found ${Object.keys(grouped).length} unique headphone models\n`
    );
    console.log("═══════════════════════════════════════════════════\n");
