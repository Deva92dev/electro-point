...
const FooterLinks = {
  shop: [
    ...
    { label: "Flash Deals", href: "/products?sort=z-a" },
  ],
  support: [
...
const Footer = () => {
  return (
    <footer className="w-full bg-background text-foreground border-t border-border pt-12 pb-8 md:pt-24 md:pb-12 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col items-center text-center mb-12 md:mb-24 space-y-6 md:space-y-8">
          ...
          <Link
            href="/products"
            className="h-12 px-8 md:h-14 md:px-10 rounded-full text-base md:text-lg font-semibold bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-primary/20 flex flex-row gap-2 items-center justify-center"
          >
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-y-12 lg:gap-12">
            ...
            <div className="flex items-center justify-center md:justify-start gap-4">
              {[Facebook, Twitter, Instagram].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all duration-300 hover:-translate-y-1"
          ...
          <div className="col-span-1 md:col-span-3 lg:col-span-2 space-y-4 text-center md:text-left">
            <h3 className="font-bold text-lg text-foreground">Shop</h3>
          ...
          <div className="col-span-1 md:col-span-12 lg:col-span-4 space-y-6 pt-4 md:pt-0 text-center md:text-left">
            <div className="space-y-4 text-center md:text-left">
              {/* COMPANY LINKS (Hidden on mobile, merged here or separate column usually, but fitting design) */}
              <h4 className="font-bold text-lg text-foreground md:hidden lg:block">
                Company
              </h4>
              <ul className="space-y-2 text-sm md:text-base md:hidden lg:block mb-6">
            ...
            <form className="relative max-w-md mx-auto md:mx-0 md:max-w-lg lg:max-w-sm group">
              <Input
                ...
                className="h-12 pr-12 rounded-full bg-muted/50 border-input text-foreground 
                  placeholder:text-muted-foreground/70 focus-visible:ring-primary focus-visible:border-primary focus-visible:bg-background
                  group-hover:border-primary/50
                  transition-all duration-300"
              />
              <button
                type="submit"
                className="absolute top-1 right-1 h-10 w-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground hover:bg-primary/90 hover:scale-105 transition-all duration-300 shadow-lg"
              >
                <Send
                  className="w-4 h-4"
        ...
        {/* COPYRIGHT */}
        <div className="mt-12 md:mt-16 pt-8 border-t border-border flex flex-col md:flex-row justify-between items-center gap-4 text-xs md:text-sm text-muted-foreground text-center md:text-left">
